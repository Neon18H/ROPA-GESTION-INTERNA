...
- `MEDIA_PUBLIC_READ=true` (default): public read for `media/*` and unsigned URLs.
- `MEDIA_PUBLIC_READ=false`: private bucket/object access with signed URLs (`AWS_QUERYSTRING_AUTH=True`).
...
- `AWS_S3_REGION_NAME`
- `MEDIA_PUBLIC_READ` (`true`/`false`, defaults to `true`)
...
```bash
python manage.py shell -c "from apps.inventory.models import Product; p=Product.objects.first(); print(p.image.url if p and p.image else 'NO_IMAGE_OR_PRODUCT')"
```
